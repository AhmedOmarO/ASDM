...
def process_rows(row):
    # Convert all values to string and join them with a space
    if isinstance(row, pd.Series):
        row_joined = ' '.join(row.values.astype(str))
        row_no_extra_spaces = row_joined.split() ## split by whitespace and remove extra spaces
        row_no_extra_spaces = ' '.join(row_no_extra_spaces) ## join back with single
        row_lower = row_no_extra_spaces.lower() ## convert to lowercase
    else:
...
class LSH:
    ...
    def make_subvecs(self, signature: np.ndarray) -> np.ndarray:
        ...
        Example:
        signature = np.array([1, 2, 3, 4, 5, 6])
        b = 3
        make_subvecs(signature)
        # Output:
        array([[1, 2],
               [3, 4],
               [5, 6]])
    ...
    def add_hash(self, signature: np.ndarray):
        ...
        This will divide the signature into 3 subvectors and add it to the corresponding buckets.
        1st band: [1, 2]
        2nd band: [3, 4]
        3rd band: [5, 6]
        1st band bucket key: "1,2"
        2nd band bucket key: "3,4"
        3rd band bucket key: "5,6"
        """
